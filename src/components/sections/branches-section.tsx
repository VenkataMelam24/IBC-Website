import { MapPin, Phone } from "lucide-react";
import { ButtonLink } from "@/components/ui/button";
import { Reveal } from "@/components/ui/reveal";

export function BranchesSection() {
  return (
    <section id="branches" className="border-t border-border bg-card py-10 md:py-20">
      <div className="mx-auto w-full max-w-7xl px-6 lg:px-10">
        <Reveal>
          <h2 className="font-heading text-3xl font-bold leading-[1.35] text-foreground md:text-5xl">
            Find Us in Berlin
          </h2>
          <div className="mt-3 h-1 w-16 bg-primary md:w-20" />
        </Reveal>

        <Reveal delay={0.08} className="mt-8 md:mt-10">
          <div className="grid items-start gap-6 rounded-2xl border border-border bg-background p-5 md:grid-cols-2 md:p-10">
            <div>
              <h3 className="font-heading text-lg font-bold text-foreground md:text-2xl">
                Indian Biryani Company
              </h3>
              <div className="mt-3 space-y-2.5 md:mt-4 md:space-y-3">
                <p className="flex items-start gap-2 text-sm text-muted-foreground md:text-base">
                  <MapPin size={15} className="mt-0.5 shrink-0 text-primary" />
                  Hektorstraße 11, 10711 Berlin, Germany
                </p>
                <p className="flex items-center gap-2 text-sm text-muted-foreground md:text-base">
                  <Phone size={15} className="shrink-0 text-primary" />
                  +49 177 3771839
                  <a href="https://wa.me/491773771839" target="_blank" rel="noopener noreferrer" aria-label="WhatsApp" className="inline-flex text-[#25d366] transition-opacity hover:opacity-70">
                    <svg viewBox="0 0 24 24" fill="currentColor" width={15} height={15}><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                  </a>
                </p>
                <p className="flex items-center gap-2 text-sm text-muted-foreground md:text-base">
                  <Phone size={15} className="shrink-0 text-primary" />
                  030 2083 3623
                </p>
              </div>
            </div>
            <div className="flex flex-wrap items-center gap-3">
              <ButtonLink href="tel:+491773771839" variant="primary">Call Now</ButtonLink>
              <ButtonLink href="https://maps.google.com/?q=Hektorstraße+11,+10711+Berlin,+Germany" variant="ghost">
                Open in Maps
              </ButtonLink>
            </div>
          </div>
        </Reveal>
      </div>
    </section>
  );
}
